...
export class EditComponent implements OnInit, AfterViewInit {
  ...
  visitors: User_AllowedInfoDto2[] = [];
  additionalVisitors: User_AllowedInfoDto2[] = [];
  availableVisitors: User_AllowedInfoDto2[] = [];
  ...
  selectedVisitor: User_AllowedInfoDto2 = {
    document: '',
    name: '',
    last_name: '',
    email: '',
  ...
  newVisitorTemplate: Partial<User_AllowedInfoDto2> = {
    document: '',
    name: '',
    last_name: '',
    email: ''
...
; 
  ...
  getDayTranslation(day: DayOfWeek): string {
  ...
  getAllowedDayHours(day: DayOfWeek): FormattedHours {
  ...
  getAvailableVisitorsForIndex(currentIndex: number): User_AllowedInfoDto2[] {
    const selectedDocuments = new Set([
      this.selectedVisitor.document,
      ...this.additionalVisitors
  ...
  updateInitHour(day: DayOfWeek, event: any): void {
    const allowedDay = this.selectedVisitor.authRange.allowedDays
      .find(d => d.day === day.toUpperCase());
    if (allowedDay) {
  ...
  updateEndHour(day: DayOfWeek, event: any): void {
    const allowedDay = this.selectedVisitor.authRange.allowedDays
      .find(d => d.day === day.toUpperCase());
    if (allowedDay) {
      const [hours, minutes] = event.target.value.split(':');
